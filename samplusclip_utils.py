...
def get_score(crop: PIL.Image.Image, texts: List[str]) -> torch.Tensor:
    ...
    return int(torch.argmax(similarity[0])), torch.max(similarity[0])
...
def get_texts(query: str) -> List[str]:
    prompts = []
    prompts.extend(f"a picture of {each_query}" for each_query in query)
    prompts.extend("a picture of background")
    return prompts
...
def filter_masks(
    ...
    for mask in sorted(masks, key=lambda mask: mask["area"])[-TOP_K_OBJ:]:
        if (
            ...
            and query
            and mask["area"] != image.shape[0] * image.shape[1]
        ):
